...
<form method="post" action="{{ route('meals.update', $meal->id) }}" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        <label>Category</label><select name="category_id" class="form-select">
    ...
    <div class="mb-3">
        <label>Image</label>
        <input type="file" name="image" class="form-control" />@if(isset($meal)&& $meal->image)<img src="{{ asset('storage/' . $meal->image) }}" width="100" class="mt-2"/>
        @endif
    </div>
